...
async function checkEngine() {
    ...
    try {
        ...
        if (response && response.status === "ok") {
            ...
            // --- ERT Hardware Trust UI ---
            const ertRow = document.getElementById('ert-row');
            const ertText = document.getElementById('ert-status-text');
            ...
            if (ertRow && response.ert_active !== undefined) {
                ...
                if (ertResealBtn && !ertResealBtn.onclick) {
                    ertResealBtn.onclick = async () => {
                        if (!confirm("This will re-anchor ClamFox to your hardware. Proceed?")) return;
                        ertResealBtn.disabled = true;
                        ertResealBtn.textContent = "...";
                        try {
                            const res = await browser.runtime.sendMessage({ action: "proxy_reseal" });
                            if (res.status === "ok") {
                                alert("Hardware Trust Refreshed Successfully.");
                                window.location.reload();
                            } else {
                                alert("Failed to refresh trust: " + res.error);
                            }
                        } catch (e) { alert("Communication Error"); }
                        ertResealBtn.disabled = false;
                        ertResealBtn.textContent = browser.i18n.getMessage("ertResealBtn") || "Refresh";
                    };
                }
            }
