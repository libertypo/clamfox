...
def _setup_i18n():
    ...
    try:
        ...
        lang = locale.getdefaultlocale()[0] or os.environ.get('LANG', 'en_US').split('.')[0]
...
_GLOBAL_TIMEOUT = 300           # 5 minute hard limit for clamscan
_NETWORK_TIMEOUT = 30           # 30 second limit for API/Fetch
...
def is_safe_path(filepath):
    ...
    try:
        ...
        for root in safe_roots:
            ...
            if real_path.startswith(root_abs):
                return True
...
def check_vpn_active():
    ...
    try:
        ...
        # 2. IP Route Check (Detect default gateway on a tunnel)
        try:
            route_process = subprocess.run(['ip', 'route', 'show', 'default'], capture_output=True, text=True, timeout=1)
            route_out = route_process.stdout.lower()
            if any(p in route_out for p in ['dev tun', 'dev wg', 'dev ppp', 'dev tap', 'dev nord', 'dev proton']):
                log_debug("VPN detected via default route on tunnel")
                return True, "VPN (Route)"
        except: pass

        # 3. Systemd-Resolve / Resolvectl Check (VPNs often add DNS servers)
        if shutil.which("resolvectl"):
            try:
                res_out = subprocess.run(['resolvectl', 'status'], capture_output=True, text=True, timeout=1).stdout.lower()
                if 'vpn' in res_out or 'tunnel' in res_out:
                    return True, "VPN (DNS)"
            except: pass

        # 4. NetworkManager Check (Very reliable on desktops)
        if shutil.which("nmcli"):
            try:
                res_out = subprocess.run(['nmcli', '-t', '-f', 'TYPE,STATE', 'connection', 'show', '--active'], capture_output=True, text=True, timeout=1).stdout.lower()
                if any(p in res_out for p in ['vpn:activated', 'tun:activated', 'wireguard:activated', 'wireguard:activat']):
                    return True, "VPN (NetworkManager)"
            except: pass

        # 5. Client Binary Specifics
        if shutil.which("protonvpn-cli"):
            try:
                p = subprocess.run(['protonvpn-cli', 'status'], capture_output=True, text=True, timeout=1)
                if "connected" in p.stdout.lower(): 
                    return True, "Proton VPN"
            except: pass
            
        if shutil.which("nordvpn"):
            try:
                p = subprocess.run(['nordvpn', 'status'], capture_output=True, text=True, timeout=1)
                if "connected" in p.stdout.lower(): 
                    return True, "NordVPN"
            except: pass
...
def check_high_threat_container(filepath, deep_scan=False):
    ...
    # 2. Use 7z to list contents (viciously fast check)
    try:
        if shutil.which("7z"):
            ...
            if process.returncode == 0:
                ...
                # Parse 7z list output to count files and malicious scripts
                for line in lines:
                ...
                # Heuristic Filter: If archive has < 15 files and contains scripts, it's highly suspect (smuggling).
                if 0 < file_count < 15 and script_count > 0:
                    return True, _("High-Threat Container: Suspicious script payload in small archive")

                # 3. Hardware-Level Scaling: Deep Scan (if requested)
                # If it's a large batch or user requested deep audit, we unpack and parallel scan.
                if deep_scan and file_count > 0:
                    log_debug(f"Hardware scaling active: Unpacking and parallel scanning {file_count} files in {filename}")
                    with tempfile.TemporaryDirectory(dir=get_run_dir()) as tmp_dir:
                        ...
                        # Parallel Scan using ProcessPool (CPU bound logic)
                        futures = [_process_pool.submit(scan_file_basic, f) for f in all_files]
                        for f in futures:
                            res = f.result()
                            if res.get("status") == "infected":
                                return True, _("Deep Parallel Scan: Infected file '{name}' found inside container ({virus})").format(name=os.path.basename(res["target"]), virus=res["virus"])
...
def handle_message(message, secret, config, stored_hash, current_hash):
    try:
        ...
        if secret and received_secret != secret:
            if action != "check":
                send_message({"status": "error", "error": _("Unauthorized: Security Handshake Failed"), "tamper": True})
                return
        ...
        if action == "ping":
            ...
            try:
                ...
                cmd = ['pkexec', 'bash', '-c', f"echo '{safe_json}' > '{config_path}'"]
            ...
            try:
                ...
                if not os.path.exists(log_file):
                    ...
                    with open(log_file, "r") as f:
                        ...
                        send_message({"status": "ok", "logs": "".join(lines)})
            except Exception as e:
                send_message({"status": "error", "error": str(e)})
