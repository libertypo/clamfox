...
async function deployDOMAnomalyShield(hvtData) {
    ...
    browser.storage.onChanged.addListener((changes, area) => {
        if (area === "local" && (changes.userWhitelist || changes.hostSecret)) {
    ...
    const observer = new MutationObserver((mutations) => {
        for (let mutation of mutations) {
            if (mutation.type === 'childList') {
                mutation.addedNodes.forEach(node => checkElement(node));
...
browser.storage.onChanged.addListener((changes, area) => {
    if (area === "local") {
        if (changes.suppressTrustedToasts || changes.userWhitelist || changes.hostSecret) {
            browser.runtime.sendMessage({ action: "get_hvts" }).then(newData => {
